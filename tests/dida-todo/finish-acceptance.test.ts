...
function completedWork(): DidaTask {
  const metadata: WorkMetadata = { schemaVersion: 3, kind: "dida-todo-work", bindingKey: scope.bindingKey, origin: "dida", lifecycle: "claimed", nextId: 3, tasks: [
...
describe("完成工作强制人类验收", () => {
  it("Repository 显式完成最后一个 Checklist 时仍可原子创建验收并完成原任务", async () => {
    ...
    remote.content = encodeManagedContent("需求说明", { schemaVersion: 3, kind: "dida-todo-work", bindingKey: scope.bindingKey, origin: "dida", lifecycle: "claimed", nextId: 3, tasks: [
      { id: 1, subject: "实现功能", status: "completed", metadata: { resolution: "实现搜索接口" } },
  ...
  it("空 Checklist 不得被误判为可完成工作", async () => {
    const remote = completedWork();
    remote.content = encodeManagedContent("需求说明", { schemaVersion: 3, kind: "dida-todo-work", bindingKey: scope.bindingKey, origin: "dida", lifecycle: "claimed", nextId: 1, tasks: [], });
    remote.items = [];
    const gateway = new FinishGateway([remote]);
    const repository = new DidaTodoRepository(gateway, new MemoryWorkStateStore());
