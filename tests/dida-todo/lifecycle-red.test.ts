...
describe("WorkLifecycle 发布阻断回归", () => {
  it("priority-0 用户草稿的 Checklist 即使全完成也绝不自动创建验收或完成", async () => {
    const metadata: WorkMetadata = { schemaVersion: 3, kind: "dida-todo-work", bindingKey: scope.bindingKey, origin: "dida", lifecycle: "draft",
    nextId: 2,
    tasks: [{ id: 1, subject: "实现", itemId: "step", status: "completed" }], };
    const gateway = new LifecycleGateway([work(metadata, { priority: 0 })]);

    await new DidaTodoRepository(gateway, new MemoryWorkStateStore()).syncOpenWorks(scope, { adoptUnmanaged: true });
  ...
  it("重复任务已推进到下一 occurrence 但 Checklist 尚未重置时不得连跳收口", async () => {
    const metadata: WorkMetadata = { schemaVersion: 3, kind: "dida-todo-work", bindingKey: scope.bindingKey, origin: "dida", lifecycle: "draft", nextId: 2, tasks: [{ id: 1, subject: "实现", itemId: "step", status: "completed" }], };
  ...
  it("Item 更新完整保留未知字段、日期与时区", async () => {
    const metadata: WorkMetadata = { schemaVersion: 3, kind: "dida-todo-work", bindingKey: scope.bindingKey, origin: "dida", lifecycle: "draft", nextId: 2, tasks: [{ id: 1, subject: "实现", itemId: "step", status: "pending" }], };
    const gateway = new LifecycleGateway([work(metadata, {
      items: [{
        id: "step",
        title: "实现",
    ...
    await new DidaTodoRepository(gateway, new MemoryWorkStateStore()).updateTask(scope, "work", 1, { status: "in_progress" });
