...
import { workTypeOf } from "./work-type.js";
...
export function formatWorkContentForAgent(work: WorkTask): string {
  ...
  return JSON.stringify({
    ...
    ...(taskType === "direct" && metadata.origin === "dida" ? {
      mustCreateVisibleChecklistStep: true,
      directTaskExecution: "Create exactly one visible step that directly completes the top-level objective, then complete it immediately when the objective is clear. Do not investigate, diagnose, split the task, or create test work.",
    } : {}),
