...
export function nextUnfinishedWork(works: WorkTask[], currentWorkId?: string): WorkTask | undefined {
  const eligible = works.filter(hasUnfinishedTasks);
  ...
  return works.slice(currentIndex + 1).find(hasUnfinishedTasks) ?? works.slice(0, currentIndex).find(hasUnfinishedTasks);
}

export function formatWorkQueueForAgent(works: WorkTask[], adoptedCount = 0, acceptances: PendingAcceptance[] = []): string {
  const unfinished = works.filter(hasUnfinishedTasks);
  ...
  return [
    "已从滴答清单同步项目 Todo。以下是全部未完成顶层工作任务，不是只处理第一项。请按顺序执行；完成一个顶层工作后继续检查并切换到下一个，直到队列为空、任务存在歧义/风险需要用户确认，或遇到无法解决的阻塞。执行过程中使用 todo 更新 Checklist 状态，并在完成时写入 metadata.resolution。",
