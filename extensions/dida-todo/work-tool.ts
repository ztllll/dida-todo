...
export function registerTodoWorkTool(pi: ExtensionAPI, repository: DidaTodoRepository, onWorkChanged: () => void): void {
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Treat each Dida work as one complete payload. For Dida-origin work, formal execution must use at least one visible Checklist Item: if a claimed task is direct with no Items, create one or more precise steps before implementation and let the repository promote it in place. Preserve every user-authored Item verbatim and only advance its status; append LLM-authored Items for clarified execution semantics. Pi-origin direct work may keep internal executionSteps. Never split one large objective into a new top-level work per phase.",
