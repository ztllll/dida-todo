...
const CURRENT_WORK = /(?:继续|接着|下一步|当前任务|当前工作|按这个来|按照这个来|就这么做|继续处理)/i;
const APPROVED_EXECUTION = /(?:^|[，,。\s])(?:对|好|可以|行|确认)?[，,。\s]*(?:按这个来|按照这个来|就这么做|直接做|开始实施|开始执行)(?:$|[，,。！!\s])/i;
const QUEUE_CHECK_LIKE = /(?:检查|查看|看看|读取|拉取|同步).{0,8}(?:todo|待办|任务清单|滴答任务)|(?:todo|待办|任务清单|滴答任务).{0,8}(?:检查|查看|看看|读取|拉取|同步)/i;

export function classifyTodoTrackingReasons(text: string): TodoTrackingReason[] {
  const value = text.trim();
  if (QUEUE_CHECK_LIKE.test(value)) return [];
